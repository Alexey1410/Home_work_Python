...
# a = set(map(int, input('Введите кол-во элементов первого множества: ').split()))
# b = set(map(int, input('Введите кол-во элементов второго множества: ').split()))
# print(*sorted(a & b))
...
lis = [ int(x) for x in input( 'Кол-во ягод через пробел: ' ).split() ]
n = len(lis)
lis = lis + lis[:2]
ma = 0
for i in range(n):
